"""Fetch R&D expenditure (% of GDP) and normalize."""
...
OUTPUT = Path(__file__).resolve().parent / "research_spending_index.csv"
API_URL = (
...
def main() -> None:
    ...
    if not data or len(data) < 2 or not data[1]:
        raise RuntimeError("No research spending data fetched")
    ...
    if not records:
        raise RuntimeError("No valid R&D records")
    ...
    if max_val == 0:
        raise RuntimeError("R&D share max is zero")
