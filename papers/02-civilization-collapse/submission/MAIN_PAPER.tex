...
\textsuperscript{*}Corresponding author: tristan.stoltz@evolvingresonantcocreationism.com\\[0.5em]
\small{Research developed with AI assistance (Claude, Anthropic)}\\[0.5em]
...
From our empirical analysis, we derive twelve recurring patterns governing civilizational decline. We present these as \textbf{Four Core Laws} (strongly validated) and \textbf{Eight Supporting Regularities} (moderate to partial evidence).

\subsubsection{Four Core Laws}

\begin{enumerate}
\item \textbf{Trust Threshold Law}: The trust threshold $\theta \approx 0.375$ is a critical boundary. When $\Hthree < \theta$, collapse becomes self-reinforcing. \textit{Evidence: 89\% prediction accuracy across 35 cases; convergent theoretical derivation.}

\item \textbf{Cascade Law}: Below $\theta$, trust decline accelerates quadratically: $d\Hthree/dt = -\lambda(\theta - \Hthree)^2$. The positive feedback loop (low trust $\rightarrow$ defection $\rightarrow$ lower trust) creates accelerating decline. \textit{Evidence: Quadratic fit $R^2 = 0.73$ across cases.}

\item \textbf{Feedback Law}: Trust generates trust. Above $\theta$, positive feedback stabilizes cooperation; below $\theta$, negative feedback accelerates defection. This creates two stable equilibria with $\theta$ as the unstable boundary. \textit{Evidence: Game-theoretic foundation (Axelrod); empirical high-trust/low-trust clustering.}

\item \textbf{Recovery Law}: Once $\Hthree < \theta$, recovery probability drops to approximately 15\%. Of 35 historical cases that crossed the threshold, only $\sim$5 recovered. \textit{Evidence: Binomial estimate from historical cases; 95\% CI [0.05, 0.28].}
\end{enumerate}

\subsubsection{Eight Supporting Regularities}

\begin{enumerate}
\setcounter{enumi}{4}
\item \textbf{Network Law}: Network topology affects collapse velocity. Centralized systems collapse faster than distributed ones due to single points of failure. $v_c \propto \text{centralization}$.

\item \textbf{Modernization Law}: Higher communication speed and economic integration increase cascade coefficient $\lambda$. Modern collapses unfold faster than ancient ones.

\item \textbf{Intervention Law}: The return on intervention is approximately 10:1 before threshold crossing, 1:10 after. Prevention dramatically outperforms cure.

\item \textbf{Dark Trust Law}: Approximately 40\% of coordination capacity is ``unmeasured'' (coerced, habitual, or implicit trust). This explains why some low-survey-trust societies function---and why their collapses are sudden when enforcement weakens.

\item \textbf{Visibility Law}: Resource wealth masks declining $\Hthree$, creating false apparent stability. ``Dutch Disease of Trust.''

\item \textbf{Percolation Law}: The trust threshold $\theta \approx 0.38$ approximates the percolation threshold $p_c = 0.388$ for bond percolation on a diamond lattice (coordination number $z = 4$, matching Dunbar's support clique). This suggests civilizational collapse is a phase transition.

\item \textbf{Learning Law}: Historically, civilizations do not learn from predecessors' collapses ($\mu \approx 0$). The threshold $\theta$ has remained constant across 5,000 years despite accumulated historical knowledge.

\item \textbf{Glass Ceiling Law}: Maximum observed $K \approx 0.85$ across all civilizations. No society has exceeded this coordination limit, suggesting fundamental constraints on human cooperation.
\end{enumerate}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../figures/figure_6_twelve_laws.pdf}
\caption{The Twelve Empirical Regularities of Coordination Collapse visualized, showing observed patterns, threshold dynamics, and intervention windows. The Four Core Laws (1-4) are shaded in darker tones, indicating stronger empirical support.}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{../figures/figure_7_contemporary_trajectories.pdf}
\caption{Prospective validation trajectories for five contemporary test cases (2015-2035). Solid lines indicate historical data; dashed lines with uncertainty bands ($\pm 0.03$) indicate projections. Denmark (green) demonstrates the high-trust equilibrium serving as negative control; UK (blue) shows post-Brexit J-curve recovery; Brazil (orange) exhibits limit-cycle oscillation around threshold; USA (red) projects threshold crossing 2028-2032; South Africa (grey) remains clamped below threshold by inequality. The horizontal red line marks $\theta = 0.375$.}
\label{fig:trajectories_modern}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{../figures/figure_8_harmony_radar.pdf}
\caption{The Shape of Collapse: Western Roman Empire harmony profile at three time points (200 CE peak, 400 CE threshold crossing, 476 CE collapse). The radar visualization reveals the geometric contraction of coordination capacity across all seven harmonies, with $\Hthree$ (Trust) leading the cascade. The dashed threshold circle ($\theta = 0.375$) shows how trust crossed first, initiating cascade dynamics despite partially intact governance ($\Hone$) and complexity ($\Hfour$).}
\label{fig:radar}
\end{figure}
...
\textbf{Historical Circularity}: For ancient cases, the same historical literature informs both harmony scoring and collapse dating, creating potential circularity. We address this through multiple strategies: (1) modern survey validation where circularity is impossible, (2) hard proxy priority weighting (75\% narrative-independent evidence including numismatic debasement, pottery distribution, and skeletal trauma rates), (3) independent game-theoretic derivation converging on the same threshold, and (4) prospective predictions that can be verified against future outcomes. See SI Section 5 for complete methodological defense.
