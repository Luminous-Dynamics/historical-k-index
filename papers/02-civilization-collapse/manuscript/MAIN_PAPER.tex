...
The seven harmonies are not independent; they interact through positive feedback loops. Figure~\ref{fig:coupling} shows the empirically-derived coupling matrix, revealing that Trust ($\Hthree$) has the highest mean coupling coefficient, serving as the ``keystone'' that amplifies or dampens changes in all other harmonies.

\begin{figure}[H]
\centering
\includegraphics[width=0.75\textwidth]{../figures/figure_3_coupling_matrix.pdf}
\caption{Harmony coupling matrix showing interdependencies between the seven coordination dimensions. Darker cells indicate stronger positive coupling. Trust ($\Hthree$) exhibits the highest mean coupling (0.72), confirming its role as the keystone harmony. The asymmetric structure reveals that trust decline propagates to all other harmonies, while economic or technological decline can be partially buffered.}
\label{fig:coupling}
\end{figure}
...
\caption{Prospective validation trajectories for five contemporary test cases (2015-2035). Solid lines indicate historical data; dashed lines with uncertainty bands ($\pm 0.03$) indicate projections. The framework predicts divergent outcomes: Denmark (green) maintains high stability; UK (blue) shows J-curve recovery from the Brexit shock; Brazil (orange) oscillates around the threshold; USA (red) is projected to cross $\theta = 0.375$ between 2028-2032; South Africa (grey) remains clamped below threshold by inequality constraints.}
...
\caption{The Shape of Collapse: Western Roman Empire harmony profile at three time points (200 CE peak, 400 CE threshold crossing, 476 CE collapse). The radar chart illustrates how the geometric mean of the seven harmonies contracts as Trust ($\Hthree$) crosses below threshold, dragging total coordination capacity toward collapse even while Governance ($\Hone$) and Complexity ($\Hfour$) remain partially intact. The dashed circle indicates $\theta = 0.375$.}
\label{fig:radar}
\end{figure}
